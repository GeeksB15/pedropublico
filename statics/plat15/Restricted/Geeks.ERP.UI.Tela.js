(function () {
  Geeks.ERP.UI.Tela = new function () {
    ...
    this.RenderObjeto = function (contentBody, objeto, utilizaPropriedadesObjeto) {
      ...
      //Botão Videoaulas
      if ((objeto.Botoes || []).find(botao => botao.Nome === 'videoaula')) {
        contentBody.find('a[name=btn_videoaula]').css('display', 'none')
        contentBody.find("a[name=btn_videoaula]").off('click')

        const myHeaders = new Headers({
          'Accept': 'application/json',
          'Content-type': 'application/json; charset=utf-8',
          'token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZFVzdWFyaW8iOiJEREIzNTJCOS1DNUMwLTQ1ODYtOTBGQy00NTlDNUMzN0VFQkQiLCJjcGYiOiIyMjUuOTExLjAwOC04OCIsImlhdCI6MTYyNzkwNTU1NywiZXhwIjoxNjU5NDQxNTU3fQ.bAr17s7XZp2YlZxZ31qazBgavdTTgWFTbK4Yo7eBy1Q',
          'debug': undefined,
          'banco': $("#hfCodigoSistema").data("GeeksData").BancoDeDados
        })

        let sistema = 'b15'
        if (~[37, 23, 42, 40, 52, 53, 57, 58, 63, 68].indexOf($("#hfCodigoSistema").data("GeeksData").CodigoSistema)) {
          sistema = 'optisoul'
        }
        ...
        const cfg = {
          ...
          body: JSON.stringify({
            tutorial: {
              funcao: '36FB3AC2-EDE6-483C-9645-FD09B69B10DA',
              componente: objeto.CodigoObjeto,
              sistema
            }
          })
        }

        fetch('https://api2.b15.com.br', cfg).then(response => {
          if (!response.ok) {
            return
          }

          response.json().then(json => {
            console.log('fetch json: ', json.tutorial)
            if (!json.tutorial.length) {
              return
            }

            contentBody.find('a[name=btn_videoaula]').css('display', 'block')

            if (json.tutorial.length === 1) {
              contentBody.find("a[name=btn_videoaula]").find('span').text('Videoaula')
              contentBody.find("a[name=btn_videoaula]").on('click', function () {
                window.open(json.tutorial[0].url, json.tutorial[0].title)
              })

              return
            }

            contentBody.find("a[name=btn_videoaula]").find('span').text('Videoaulas')
            contentBody.find("a[name=btn_videoaula]").on('click', function () {
              let promptSql = ''

              for (const el of json.tutorial) {
                if (promptSql) {
                  promptSql += ' union '
                }
              ...
              const prompt = new Geeks.ERP.PromptB(this, "Videoaulas", promptSql)
              prompt.Configuracao(1, "Código", 70, 1, "Codigo", "numero", '', 0)
              prompt.Configuracao(2, "Vídeo", 250, 0, "title", "texto", "asc")
              prompt.Configuracao(3, "url", 300, 1, "url", "texto", '', 0)

              prompt.Show(function (objeto) {
                window.open(objeto.url, objeto.title)
              })
            })
          })
        })
